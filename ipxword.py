...
class IPXWordGenerator:
    def __init__(self, G, numk=-1, verbose=False, wordfile='words.txt'):
        self.G = G
        ...
        for i in range(self.N):
            for j in range(self.N):
                ...
                for let in 'abcdefghijklmnopqrstuvwxyz':
                    ...
                    prob += pulp.lpSum(self.zvars[(k1,slota)] for k1 in rowx) == \
                            pulp.lpSum(self.zvars[(k2,slotd)] for k2 in colx)
                    
        # Objective : random costs for now
        prob += pulp.lpSum(self.allcosts[k] * self.zvars[(k,s)] \
                      for k in range(self.numk) for s in self.slots)

        # PULP_CBC_CMD is to mute the excessive output
        status = prob.solve(pulp.PULP_CBC_CMD(msg=0))
        if pulp.LpStatus[status] == "Infeasible":
            raise ValueError("no, dingus, you can't do that")
        else:
            if verbose:
                print('Puzzle status:', pulp.LpStatus[status])
    ...
    def get_puzzle(self):
        ...
        for k in range(self.numk):
            for s in self.slots:
                if pulp.value(self.zvars[(k,s)]) > 0:
                    count += 1
...
if __name__ == '__main__':
    ...
    ipx = IPXWordGenerator(G, numk=500, wordfile='beans.txt')
    ipx.build()
    ipx.get_puzzle()
